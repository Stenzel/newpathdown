...
void resampler::buildsinc(void)
{
    ...
    for(int i=0; i<=256; i++)
    {
        ...
        for(int k=0; k<8; k++)
        {
            ...
            s *= 0.52 - 0.48 * cos(warg);         // window
            sinc[k] = s;
            sum  += s;                            // sum for gain correction
            
            arg  += step;                         // increment sinc arg
            warg += wstep;                        // increment window arg
        }
        
        double gain_correction = 1.0/sum;
        for(int k=0; k<8; k++) Sinc[i][k] = sinc[k] * gain_correction;
    }
}
